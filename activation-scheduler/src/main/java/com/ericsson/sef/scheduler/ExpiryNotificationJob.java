...
public class ExpiryNotificationJob implements Job {
	...
	public void execute(JobExecutionContext context) throws JobExecutionException {
		try {
			...
			log.info("ExpiryNotificationJob executing job with job ID : " + jobId + " JOb details : "
					+ context.getJobDetail().getJobDataMap().toString());

			final ScheduledRequest scheduledRequest = SefCoreServiceResolver.getScheduleRequestService().getScheduledRequestByJobId(jobId);
			...
			if (scheduledRequest.getStatus() != ScheduledRequestStatus.SCHEDLUED) {
				...
				throw new RuntimeException("ExpiryNotification job is not set to scheduled for the given executing job with job ID: "
						+ jobId);
			}

			SubscriberInfo subscriberInfo = TransactionEngineHelper.getSubscriberInfo(scheduledRequest.getUserId());

			if (subscriberInfo == null)
				throw new RuntimeException("User with user ID  does not exist anymore: " + scheduledRequest.getUserId());

			Collection<ScheduledRequestMeta> metas = SefCoreServiceResolver.getScheduleRequestService().getScheduledRequestMetas(
