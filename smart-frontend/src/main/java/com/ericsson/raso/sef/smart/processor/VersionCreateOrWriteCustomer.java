...
public class VersionCreateOrWriteCustomer implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		Thread.sleep(200); // hold on!!
		VersionCreateOrWriteCustomerRequest versionCreateOrWriteCustomerRequest = (VersionCreateOrWriteCustomerRequest) exchange.getIn()
				.getBody();
		logger.debug("Got it!");
		...
		SubscriberInfo subscriberInfo = updateSubscriber(requestId, versionCreateOrWriteCustomerRequest.getCustomerId(), metas,
				Constants.VersionCreateOrWriteCustomer);
		// exchange.getOut().setBody(subscriberInfo);
		if (subscriberInfo.getStatus() != null && subscriberInfo.getStatus().getCode() > 0) {

			throw ExceptionUtil.toSmException(new ResponseCode(subscriberInfo.getStatus().getCode(), subscriberInfo.getStatus()
					.getDescription()));
	...
	private SubscriberInfo updateSubscriber(String requestId, String customer_id, List<Meta> metas, String useCase) throws SmException {
