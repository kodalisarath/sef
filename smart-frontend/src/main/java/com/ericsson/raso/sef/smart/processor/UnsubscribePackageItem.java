	...
	public void process(Exchange exchange) throws Exception {
		...
		if (usecase == 1) {
			...
			if (subscriberResponse.getStatus() != null && subscriberResponse.getStatus().getCode() > 0) {
				...
				throw ExceptionUtil.toSmException(new ResponseCode(subscriberResponse.getStatus().getCode(), subscriberResponse.getStatus()
						.getDescription()));
			} else {
			...
			logger.info("SK GET METAS: " + subscriberMetas);
			...
			if (inRecycle == false) {
				...
				if (purchaseResponse.getFault() != null && purchaseResponse.getFault().getCode() > 0) {
					...
					logger.debug("");
					throw ExceptionUtil.toSmException(new ResponseCode(purchaseResponse.getFault().getCode(), purchaseResponse.getFault()
							.getDescription()));
				} else {
					CommandResponseData cr = this.createResponse(true);
					String edrIdentifier = (String) exchange.getIn().getHeader("EDR_IDENTIFIER");
