...
public class DefaultSmppServerHandler implements SmppServerHandler {
	...
	public void sessionBindRequested(Long sessionId,
			...
			@SuppressWarnings("rawtypes") BaseBind bindRequest)
			throws SmppProcessingException {
		if (!sessionConfiguration.getSystemId().equals(systemId)
				|| !sessionConfiguration.getPassword().equals(password)) {
		...
		log.info("sessionBindRequested:: sessionID: " + sessionId
	...
	public void sessionCreated(Long sessionId, SmppServerSession session,
		...
		log.info("sessionCreated:: sessionID: " + sessionId
				+ " preparedBindResponse:: " + preparedBindResponse);
	...
	public void sessionDestroyed(Long sessionId, SmppServerSession session) {
		...
		if (session.hasCounters()) {
			log.info(" final session rx-submitSM: {}", session.getCounters()
					.getRxSubmitSM());
		}
