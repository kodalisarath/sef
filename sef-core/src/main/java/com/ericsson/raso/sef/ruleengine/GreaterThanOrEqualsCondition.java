...
public final class GreaterThanOrEqualsCondition extends Condition {
	private static final long serialVersionUID = 431251081265808826L;

	// TODO: add logger initialization
	private Object refValue = null;

	public GreaterThanOrEqualsCondition(Object ruleValue) {
		...
		if (ruleValue instanceof Number || ruleValue instanceof Date || ruleValue instanceof Calendar
			...
			this.setType(ConditionType.GREATER_THAN_OR_EQUALS);
	...
	public boolean evaluate(Object operand) throws RuleFailedException {
		...
		if (operand instanceof Number && this.refValue instanceof Number) {
			...
			if (((Number) this.refValue).doubleValue() <= ((Number) operand).doubleValue()) {
				...
				// "' is greater or equals Operand: '" + operand);
				return true;
			} else {
				// TODO: logger.debug("Rule Value: '" + this.refValue +
				// "' is not greater or equals Operand: '" + operand);
				return false;
			}
		}
		...
		if (operand instanceof Comparable && this.refValue instanceof Comparable) {
			...
			if (((Comparable) this.refValue).compareTo(operand) <= 0) {
				...
				// TODO: logger.debug("Rule Value: '" + this.refValue+
				// "' is not greater or equals Operand: '" +
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		GreaterThanOrEqualsCondition other = (GreaterThanOrEqualsCondition) obj;
		if (refValue == null) {
	...
	public String toString() {
		return " is Greater Than Or Equals '" + refValue + "'";
	}
