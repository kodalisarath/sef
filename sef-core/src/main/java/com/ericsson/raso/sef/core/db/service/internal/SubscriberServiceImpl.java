...
public class SubscriberServiceImpl implements SubscriberService {
	...
	public boolean createSubscriber(String nbCorellator, Subscriber subscriber)
		...
		try {
			subscriber.setAccountId(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getAccountId()))));
			subscriber.setContractId(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getContractId()))));
			subscriber.setCustomerId(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getCustomerId()))));
			subscriber.setEmail(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getEmail()))));
			subscriber.setImeiSv(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getImeiSv()))));
			subscriber.setImsi(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getImsi()))));
			subscriber.setMsisdn(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getMsisdn()))));
			subscriber.setPaymentParent(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getPaymentParent()))));
			subscriber.setPaymentResponsible(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getPaymentResponsible()))));
			subscriber.setPin(new String(org.apache.commons.codec.binary.Base64
					.encodeBase64(encryptor.encrypt(subscriber.getPin()))));
			subscriber.setUserId(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getUserId()))));
			...
			logger.error(
					...
					e);
			throw new PersistenceError(nbCorellator, this.getClass().getName(),
					new ResponseCode(InfrastructureError,
							"Failed to encrypt Subscriber identities!!"), e);
		}

		try {
			logger.debug("Crossing fingers... About to insert subscriber:"
					+ subscriber);
			subscriberMapper.createSubscriber(subscriber);
			this.createMetas(nbCorellator, subscriber.getUserId(),
					subscriber.getMetas());

		} catch (PersistenceException e) {
			logger.error("Encountered Persistence Error. Cause: "
					+ e.getCause().getClass().getCanonicalName(), e);
			throw new PersistenceError(nbCorellator, this.getClass().getName(),
					new ResponseCode(InfrastructureError,
	...
	public boolean deleteSubscriber(String nbCorrelator, String userId)
		...
		try {
			subscriberId = new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(userId)));
		} catch (FrameworkException e) {
			logger.error(
					nbCorrelator,
					"Could not prepare entity for persistence. Cause: Encrypting Identities",
		...
		try {
			...
			logger.debug(nbCorrelator, "Subscriber: " + userId
			...
			throw new PersistenceError(nbCorrelator, this.getClass().getName(),
					new ResponseCode(InfrastructureError,
							"Failed to delete Subscriber entity!!"), e);
		}
	...
	public boolean updateSubscriber(String nbCorrelator, Subscriber subscriber)
		...
		try {
			...
			subscriber.setUserId(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(subscriber
									.getUserId()))));
			logger.debug(nbCorrelator,
					"Encrypted Identities to ensure info security");
		} catch (FrameworkException e) {
			logger.error(
					nbCorrelator,
					"Could not prepare entity for persistence. Cause: Encrypting Identities",
		...
		try {
			...
			logger.error("Encountered Persistence Error. Cause: "
					+ e.getCause().getClass().getCanonicalName(), e);
			throw new PersistenceError(nbCorrelator, this.getClass().getName(),
					new ResponseCode(InfrastructureError, e.getMessage()), e);
		}

		// select the list of changes to audit trail...
		// Collection<SubscriberAuditTrial> newAuditTrails = new
		// ArrayList<SubscriberAuditTrial>();

		// if(currentEntity.getContractState() != null &&
		// currentEntity.getContractState() != subscriber.getContractState()) {
		// SubscriberAuditTrial historyEvent = new
		// SubscriberAuditTrial(subscriber.getUserId(),
		// new Date(),
		...
		// boolean
		// isMetasSet=setMeta(nbCorrelator,subscriber.getUserId(),listMetas);
		// } catch (PersistenceException e) {
		// logger.error("Encountered Persistence Error. Cause: " +
		// e.getCause().getClass().getCanonicalName(), e);
		// throw new PersistenceError(nbCorrelator, this.getClass().getName(),
	...
	// Made this method as a private method to invoke when a subscriber is
	// created or updated

	private boolean setMeta(String nbCorrelator, String userId, List<Meta> metas)
			throws PersistenceError {
		...
		for (Meta meta : metas) {
			...
			for (Meta currentMeta : currentMetas) {
				if (meta.getKey().equals(currentMeta.getKey())) {
					/**
					 * Audit Trail will be ignored temporarily until all core
					 * requisite functionalit is out...
					 */
					// SubscriberAuditTrial history = new
					// SubscriberAuditTrial(userId,
					// new Date(),
					// meta.getKey(),
					// meta.getValue(),
					// "system-user");
					try {
						...
						throw new PersistenceError(nbCorrelator, this
								...
								InfrastructureError, e.getMessage()), e);
					} catch (FrameworkException e) {
						logger.error(
								nbCorrelator,
								"Could not prepare entity for persistence. Cause: Encrypting Identities",
	...
	@Override
	@Transactional
	public boolean updateMetas(String nbCorrelator, String userId,
		...
		for (Meta meta : metas) {
			...
			for (Meta currentMeta : currentMetas) {
				if (meta.getKey().equals(currentMeta.getKey())) {
					/**
					 * Audit Trail is ignored until core functionality is
					 * thru...
					 */
					// SubscriberAuditTrial history = new
					// SubscriberAuditTrial(userId,
	...
	public List<Meta> getMetas(String nbCorrelator, String userId,
		...
		/*
		 * if (subscriber == null) throw new PersistenceError(nbCorrelator,
		 * this.getClass().getName(), new ResponseCode(ApplicationContextError,
		 * "A subscriber meta cannot be found for the given 'userId'!!"));
		 */
	...
	public Subscriber getSubscriber(String nbCorrelator, String userId,
		 ...
		 * if (subscriber == null) throw new PersistenceError(nbCorrelator,
		 * this.getClass().getName(), new ResponseCode(ApplicationContextError,
		 * "A subscriber cannot be found for the given 'userId'!!"));
		 */
	...
	public List<SubscriberAuditTrial> getSubscriberHistory(String nbCorrelator,
		...
		Collection<SubscriberAuditTrial> subscriberHistory = subscriberMapper
				.getSubscriberHistory(map);
	...
	private Subscriber fetchSubscriberByMsisdn(String nbCorrelator,
		...
		try {
			subscriberId = new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(msisdn)));
		} catch (FrameworkException e) {
			logger.error(
					nbCorrelator,
					"Could not prepare entity for persistence. Cause: Encrypting Identities",
		...
		if (subscriber != null) {
			Collection<SubscriberMeta> subscriberMetas = this
					.fetchMetas(msisdn);
			...
			subscriber.getMetas().add(
					new Meta("SUBSCRIBER_ID", subscriber.getUserId()));
		}
		return subscriber;
	}

	private Subscriber fetchSubscriberByUserId(String nbCorrelator,
			String userId) throws PersistenceError {
	...
	private Collection<SubscriberMeta> fetchMetas(String userId) {
		logger.debug("Inside the method to fetch metas for subscriber: "
				+ userId);

		Collection<SubscriberMeta> metas = null;
		try {
			metas = subscriberMapper.getAllSubscriberMetas(new String(
					org.apache.commons.codec.binary.Base64
							.encodeBase64(encryptor.encrypt(userId))));
		} catch (PersistenceException e) {
			logger.error(
					"DB execution failed fetching metas. Cause: "
							+ e.getMessage(), e);
		} catch (FrameworkException e) {
			logger.error(
					"DB preparation failed fetching metas. Cause: "
							+ e.getMessage(), e);
	...
	private List<SubscriberAuditTrial> translateHistory(
			Collection<SubscriberAuditTrial> history) {
	...
	public boolean isSubscriberExist(String subscriber) throws PersistenceError {
		...
		if (subscriber == null) {
			...
			throw new PersistenceError(null, this.getClass().getName(),
					new ResponseCode(ApplicationContextError,
							"The 'msisdn' provided was null!!"));
