...
public final class LesserThanCondition extends Condition {
	private static final long serialVersionUID = 4184196895398618137L;

	// TODO: add logger initialization
	private Object refValue = null;

	public LesserThanCondition(Object ruleValue) {
		...
		if (ruleValue instanceof Number || ruleValue instanceof Date || ruleValue instanceof Calendar
			...
			this.setType(ConditionType.LESSER_THAN);
	...
	public boolean evaluate(Object operand) throws RuleFailedException {
		...
		if (operand instanceof Number && this.refValue instanceof Number) {
			...
			if (((Number) this.refValue).doubleValue() > ((Number) operand).doubleValue()) {
				...
				// "' is lesser Operand: '" + operand);
				return true;
			} else {
				// TODO: logger.debug("Rule Value: '" + this.refValue +
				// "' is not lesser Operand: '" + operand);
				return false;
			}
		}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		LesserThanCondition other = (LesserThanCondition) obj;
		if (refValue == null) {
	...
	public String toString() {
		return " is Lesser Than '" + refValue + "'";
	}
