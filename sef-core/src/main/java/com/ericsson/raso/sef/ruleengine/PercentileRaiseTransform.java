...
public final class PercentileRaiseTransform extends Transform {
	private static final long serialVersionUID = 1871546554762665822L;

	// TODO: add logger initialization

	private Number percentile = null;

	public PercentileRaiseTransform(Number percentile) {
		...
		super.setType(TransformType.PERCENTILE_RAISE);
	...
	public Object apply(Object operand) throws TransformFailedException {
		...
		if (operand instanceof BigInteger) {
			if (this.percentile instanceof BigInteger) {
				return ((BigInteger) operand).multiply(BigInteger.valueOf(1).add(
						((BigInteger) this.percentile).divide(BigInteger.valueOf(100))));
			}

			if (this.percentile instanceof BigDecimal) {
				return ((BigDecimal) operand).multiply(BigDecimal.valueOf(1).add(
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		PercentileRaiseTransform other = (PercentileRaiseTransform) obj;
		if (percentile == null) {
	...
	public String toString() {
		return "Increase By Percentile [percentage '" + percentile + "']";
	}
