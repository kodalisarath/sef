...
public final class SubtractTransform extends Transform {
	private static final long serialVersionUID = 1066061884672503523L;

	// TODO: add logger initialization

	private Number offset = null;

	public SubtractTransform(Number offset) {
		if (!(offset != null && offset instanceof Number)) {
			// TODO: logger.debug("Rule Value: is null!!");
			throw new IllegalArgumentException("Found 'null' or invalid type argument");
		}

		super.setType(TransformType.SUBTRACT);
	...
	public Object apply(Object operand) throws TransformFailedException {
		...
		if (operand instanceof BigInteger) {
			if (this.offset instanceof BigInteger) {
				return ((BigInteger) operand).subtract((BigInteger) this.offset);
			}

			if (this.offset instanceof BigDecimal) {
				return ((BigDecimal) operand).subtract((BigDecimal) this.offset);
			} else {
				// TODO:
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		SubtractTransform other = (SubtractTransform) obj;
		if (offset == null) {
	...
	public String toString() {
		return "Subtract [decrement '" + offset + "']";
	}
