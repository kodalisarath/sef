...
public class OfferContainer implements Serializable {
	...
	/**
	 * hierarchical store for managing offers; with the structure: Offer Name
	 * --> Version --> Offer Instance
	 */
	private TreeMap<String, TreeMap<Integer, Offer>> offersById = new TreeMap<String, TreeMap<Integer, Offer>>();
	/**
	 * hierarchical store for mapping resources to offers - to enable quick
	 * discoveries; use of identifiers in String is ensure memory leak specially
	 * serialization is involved. structure: Resource Name --> List of Offer
	 * Names
	 */
	private TreeMap<String, TreeSet<String>> offersByResourceName = new TreeMap<String, TreeSet<String>>();

	/**
	 * simple mapping store to identify the products with external handles; this
	 * will allow federation scenarios to external catalogs to rapidly identify
	 * offer in our catalog.
	 */
	private TreeMap<String, String> offersByExternalHandle = new TreeMap<String, String>();

	private static final Logger logger = LoggerFactory
			.getLogger(OfferContainer.class);
	...
	public void createOffer(Offer offer) throws CatalogException {
		...
		if (this.offerExists(offer.getName()))
			throw new CatalogException(
					"Offer already exists!! Use updateOffer() instead, which can manage versions, if this offer is a variant!");
		...
		synchronized (this) {
			...
			for (Product product : offer.getProducts()) {
				if (product instanceof AtomicProduct) {
					String resourceName = ((AtomicProduct) product)
							.getResource().getName();
					TreeSet<String> offers = this.offersByResourceName
							.get(resourceName);
					if (offers == null)
				...
				if (product instanceof ProductPackage) {
					for (AtomicProduct atomicProduct : ((ProductPackage) product)
							.getAtomicProducts()) {
						String resourceName = atomicProduct.getResource()
								.getName();
						TreeSet<String> offers = this.offersByResourceName
								.get(resourceName);
						if (offers == null)
	...
	public void updateOffer(Offer offer) throws CatalogException {
		...
		if (!this.offerExists(offer.getName()))
			throw new CatalogException("Offer: " + offer.getName()
		...
		if (version2update < versionFromStore)
			throw new CatalogException(
					"Version is prepopulated with value: "
							+ version2update
							+ ". Seems like a previous version is being edited . "
		...
		if (!lastKnownOfferRevision.checkEditSanity(offer))
			throw new CatalogException(
					"Seems like critical core elements of Offer: "
							+ offer.getName()
		...
		if (lastKnownOfferRevision.getHistory().getPublishedTime() > -1) {
			// /// check if any active subscriptions exist for the latest
			// version....
			try {
				Integer activeSusbriptions = new GetActiveSubscriptionCountForOffer(
						offer.getName(), version2update).execute();
				...
				if (activeSusbriptions >= 1) {
					...
					if (offer.getHistory() == null)
						offer.setHistory(new OfferLifeCycle());
					offer.getHistory().setCreationTime(
							System.currentTimeMillis());

				} else {
					// can keep the existing version..
					offer.setVersion(versionFromStore); // technically this dead
														// code since there is
														// no impact on this
														// assignment....
				}
				...
				throw new CatalogException(
						"Unable to assert no active subscriptions exist for this offer. Try later!!",
						e);
			}
		}
		...
		synchronized (this) {
			// first manage offer....
			TreeMap<Integer, Offer> versionedOffer = this.offersById.get(offer
					.getName());
			versionedOffer.put(offer.getVersion(), offer);
			this.offersById.put(offer.getName(), versionedOffer);
	...
	/**
	 * This is purely housekeeping method and must never be called from GUI. Use
	 * changeLifeCycle() instead to put the offer in RETIRED state. Not adhering
	 * to this directive, will cause a lot of customer disputes with active
	 * subscriptions.
	 * 
	 * @param offerId
	...
	public void deleteOffer(String offerId) throws CatalogException {
		...
		synchronized (this) {
			TreeMap<Integer, Offer> versionedOffer = this.offersById
					.get(offerId);
			...
			if (this.offersByExternalHandle.containsValue(offerId)) {
				for (String handle : this.offersByExternalHandle
						.descendingKeySet())
			...
			if (this.offersByResourceName.containsValue(offerId)) {
				for (String resourceName : this.offersByResourceName
						.descendingKeySet())
					if (this.offersByResourceName.get(resourceName).equals(
							offerId))
	...
	public void changeLifeCycle(String offerId, int vesion, State newState)
			throws CatalogException {
	...
	public Offer getOfferById(String id) {
		...
		if (versionedOffer != null) {
			...
			logger.debug("Native Concrete Instance Offer Name: "
					+ offer.getName());
		}
	...
	public void listAllOffers() {
		if (offersById != null) {
			for (String offerKey : offersById.keySet()) {
				TreeMap<Integer, Offer> offers = offersById.get(offerKey);
				if (offers != null) {
					for (Integer offerId : offers.keySet()) {

						Offer offer = offers.get(offerId);
						if (offer != null) {
							logger.debug("offerKey: " + offerKey
									+ " ,offerId: " + offerId + " ,offerName:"
									+ offer.getName() + " ,ExternalHandles: "
									+ offer.getExternalHandles());

						} else {
							logger.debug("We might have a problem offer is null for the offerId "
									+ offerId);
						}
					}
				} else {
					logger.debug("We might have a problem offers is null for the key "
							+ offerKey);
				}
			}
		} else {
			logger.debug("We have a problem offersByid is null");
		}
