...
import com.ericsson.sm.core.diameter.ChargingInfo;
...
public class TestClient {
	...
	public static void testChargingGateway() throws Exception {
		...
		// localIP
		builder.ownStackUri("aaa://10.132.133.29:3868;transport=tcp");
		builder.ownProductId("ILCG");
		builder.ownRealm("smart.com.ph");
		builder.tcpPort(3868);

		// target IP
		builder.addStaticRoute("smart.com.ph",
				"aaa://10.202.160.17:3868;transport=tcp");
		...
		// Cca cca =
		// createGetConsumerAccListRequest(stack.getDiameterStack()).send()
		// ;//ccrMessage.send();
	...
	public static void main(String[] args) {
		...
		try {
			testAuthorise("639777000011", "2", "10.202.160.8");
		} catch (Exception e) {
		 ...
		 * DiameterUtil.toMap(chargingInfo.getAvpList());
		 * System.out.println(map.size());
		 */
		System.exit(0);
	}

	public static void testAuthorise(String msisdn, String obj, String server)
			throws Exception {
		DiameterStackBuilder builder = new DiameterStackBuilder(Stack.SCAPV2);

		// localIP
		builder.ownStackUri("aaa://10.111.15.8:3868;transport=tcp");
		builder.ownProductId("ILCG");
		builder.ownRealm("smart.com.ph");
		builder.tcpPort(3868);

		// target IP
		builder.addStaticRoute("smart.com.ph", "aaa://" + server
				+ ":3868;transport=tcp");
		...
		Ccr ccrMessage = new Ccr("NSN1234567890", stack.getDiameterStack(),
				ScapChargingApi.Service_Context_Id);
		...
		ccrMessage.setEventTimestamp(new Time(new Date(System
				.currentTimeMillis())));
	...
	public static void testAll() throws Exception {
		...
		System.out.println(ccrMessage.getSessionId());
		// target IP
		ccrMessage.setDestinationHost("10.202.160.17");
		ccrMessage.setEventTimestamp(new Time(new Date(System
				.currentTimeMillis())));
	...
	public static Ccr createGetConsumerAccListRequest(DiameterStack stack) {
		Ccr ccrMessage = new Ccr("NSN1234567890", stack,
				ScapChargingApi.Service_Context_Id);

		ccrMessage.setSessionId(String.valueOf(1234217890/*
	...
	public static Ccr createCommitRequest(DiameterStack stack) {
		...
		ccrMessage.setSessionId(String.valueOf(1234517990/*
														 * System.currentTimeMillis
														 * ()
														 */));
		ccrMessage.setDestinationRealm("smart.com.ph");
	...
	private static ChargingInfo readObject(String objectId) throws Exception {
		InputStream stream = TestClient.class.getClassLoader()
				.getResourceAsStream("requests/" + objectId);
	...
	public static Map<String, Object> toMap(Collection<Avp> avps) {
		...
		for (Avp avp : avps) {
			...
			if (list == null) {
				map.put(String.valueOf(avp.getAvpCode()),
						new String(avp.getData()));
			} else {
	...
	public static void testGetConsumerAcc(String msisdn, String server)
			throws Exception {
		DiameterStackBuilder builder = new DiameterStackBuilder(Stack.SCAPV2);
