...
public class SubscriberValidationProcessor implements Processor {
	...
	public void process(Exchange exchange) throws Exception {
		...
		log.debug("Inside SubscriberValidationProcessor Operation is  "
				+ operation);
		if (Operation.DIRECT_DEBIT != operation
				&& Operation.RESERVE != operation
				&& Operation.MMS_CHARGING != operation
				&& Operation.SMS_CHARGING_INITIAL != operation
				&& Operation.SMS_CHARGING_TERMMINATE != operation) {
			if (log.isDebugEnabled())
				log.debug("This Operation does not required subscriber Validation .skipping it and moving to next Processor.. ");

			return;
		}
		else {
			if (log.isDebugEnabled())
				log.debug("Operation requires subscriber Validation .About to call read Subscriber. ");

		}
		String messageId = (String) exchange.getIn().getHeader(
				CgConstants.messageId);
		String msisdn = request.getMsisdn();
		log.info("	Start validating subsciber for message Id " + messageId
				+ " msisdn is :" + msisdn);
		...
		try {
			...
			log.info(" CGEngine SubscriberValidationProcessor subscriberInfo for message Id "
					+ messageId
					+ " &  msisdn "
					+ msisdn
					+ " subscriberInfor is " + subscriberInfo);

		} catch (SmException e) {
			log.error(
					"Exception in ChargingGatewayEngine.SubscriberValidationProcessor "
							+ e.getMessage(), e);
		...
		//
		// if(subscriberInfo.getMetas() !=null &&
		// subscriberInfo.getMetas().containsKey(metaKey) &&
		// subscriberInfo.getMetas().get(metaKey) !="0")
		// {
		// log.error("Subscriber:" + msisdn + " does not have Tagging Meta.");
		// throw new SmException(ResponseCode.SUBSCRIBER_LOCKED);
		// }

		String activeStatusCS = subscriberInfo.getSubscriber().getMetas()
				.get("READ_SUBSCRIBER_ACTIVATION_STATUS_FLAG");
		...
		Map<String, String> subscriberMetas = subscriberInfo.getSubscriber()
				.getMetas();
		...
		for (String key : subscriberMetas.keySet()) {
			log.debug("FLEXI:: processing meta:" + key + "="
					+ subscriberMetas.get(key));
			if (key.startsWith(READ_SUBSCRIBER_OFFER_INFO_OFFER)) {
				...
				oInfo = new OfferInfo(offerId, Long.parseLong(expiry),
						Long.parseLong(start), null, null);
	...
	class OfferInfo {
		...
		public OfferInfo(String offerID, long offerExpiry, long offerStart,
				String daID, String walletName) {
			super();
		...
		public String toString() {
			return "OfferInfo [offerID=" + offerID + ", offerExpiry="
					+ offerExpiry + ", daID=" + daID + ", walletName="
					+ walletName + "]";
		}
