...
import com.ericsson.raso.sef.core.cg.nsn.avp.mms.RecipientAddressDomainAvp;
...
public class AdressAvp extends DccGrouped {
	...
	public ServiceParameterInfoAvp getOriginAdressDataAvp() {
		...
		if(avp == null) return null;
		ServiceParameterInfoAvp info = new ServiceParameterInfoAvp();
		info.addSubAvp(new ServiceParameterTypeAvp(253));
		info.addSubAvp(new ServiceParameterValueAvp(avp.getData()));
		return info;
	}
	...
	public ServiceParameterInfoAvp getRecipientAdressDomainNameAvp() throws AvpDataException {
		Avp avp = getSubAvp(RecipientAddressDomainAvp.AVP_CODE);
		if(avp==null) return null;
		RecipientAddressDomainAvp recAddDomainAvp = new RecipientAddressDomainAvp(avp);
		Avp domainNameAvp = recAddDomainAvp.getDomainName();
		if(domainNameAvp==null) return null;
		ServiceParameterInfoAvp info = new ServiceParameterInfoAvp();
		info.addSubAvp(new ServiceParameterTypeAvp(252));
		info.addSubAvp(new ServiceParameterValueAvp(domainNameAvp.getData()));
		return info;
	}
}
